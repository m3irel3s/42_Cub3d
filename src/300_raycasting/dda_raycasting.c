...
/*   Updated: 2025/04/09 22:37:18 by meferraz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_perform_dda(t_game *game, t_ray *ray)
{
	while (ray->hit == 0)
	{
		...
		if (ray->map_y < 0 || ray->map_y >= (int)game->map->height)
		{
			ray->hit_outside = 1;
			ray->hit = 1;
			break;
		}
		int current_row_len = ft_strlen(game->map->grid[ray->map_y]);
		if (ray->map_x < 0 || ray->map_x >= current_row_len)
		{
			ray->hit_outside = 1;
			ray->hit = 1;
			break;
