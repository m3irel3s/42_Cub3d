...
/*   Updated: 2025/04/12 16:36:08 by meferraz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_calc_minimap_scale(t_game *game)
{
	...
	game->map->minimap.scale = (SCREEN_HEIGHT / game->map->height) \
		* MINIMAP_RATIO;
	game->map->minimap.start_x = SCREEN_WIDTH / 2 - \
...
static void	ft_draw_player(t_game *game)
{
	...
	size = game->map->minimap.scale / 4;
	pos.x = game->map->minimap.start_x + (int)(game->player->pos_x \
		* game->map->minimap.scale) - size / 2;
	pos.y = game->map->minimap.start_y + (int)(game->player->pos_y \
		* game->map->minimap.scale) - size / 2;
	ft_mlx_pixel_put_to_image(game, pos.x + size / 2, \
		pos.y + size / 2, COLOR_PLAYER);
}
